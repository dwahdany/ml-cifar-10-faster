...
def build_graph(net, path_map="_".join):
    net = {
        path: node if len(node) == 3 else (*node, None) for path, node in path_iter(net)
    }
    default_inputs = chain([("input",)], net.keys())
    resolve_path = (
        lambda path, pfx: pfx + path
        if (pfx + path in net or not pfx)
        else resolve_path(net, path, pfx[:-1])
    )
    return {
        path_map(path): (
            typ,
            ...
            (
                [path_map(default)]
                if inputs is None
                else [path_map(resolve_path(make_tuple(k), path[:-1])) for k in inputs]
            ),
        )
...
def whitening_block(c_in, c_out, eigen_values=None, eigen_vectors=None, eps=1e-2):
    ...
    filt.weight.data = (
        eigen_vectors / torch.sqrt(eigen_values + eps)[:, None, None, None]
    )
...
def forward_tta(tta_transforms, model, batch, loss):
    ...
    logits = torch.mean(
        torch.stack(
            [
                model({"input": transform(batch["input"].clone())})["logits"].detach()
...
def eval_on_batches(model, loss, batches):
    """Evaluate model on batches using the given loss function."""
    eval_log = []
    model.eval()
    with torch.no_grad():
        for batch in batches:
            ...
            for k, v in out.items():
                eval_log.append((k, v.detach()))
    res = map_values(
        (lambda xs: to_numpy(torch.cat(xs)).astype(float)), group_by_key(eval_log)
    )
    return mean_members(res)
...
class Network(nn.Module):
    def __init__(self, net, loss=None):
        super().__init__()
        self.graph = {
            path: (typ, typ(**params), inputs)
            for path, (typ, params, inputs) in build_graph(net).items()
        }
...
class SGDOpt(object):
    ...
    def __init__(
        self, weight_param_schedule, bias_param_schedule, weight_params, bias_params
    ):
    ...
    def step(self):
        ...
        # The weights
        param_values = {
            k: f(self.step_number) for k, f in self._w_param_schedule.items()
        }
        ...
        param_values = {
            k: f(self.step_number) for k, f in self._b_param_schedule.items()
        }
...
def compute_patch_whitening_statistics(train_set):
    ...
    def eigens(patches):
        ...
        eigen_values, eigen_vectors = torch.linalg.eigh(covariance, UPLO="L")
        return eigen_values.flip(0), eigen_vectors.t().reshape(c * h * w, c, h, w).flip(
            0
        )
...
def transformed(
    data, targets, transform, max_options=None, unshuffle=False, device=None
):
